...
def get_dataset():
    ...
    train_set = torchaudio.datasets.SPEECHCOMMANDS('./data', download=True)
    
    # Extract labels
    labels = sorted(list(set(datapoint[2] for datapoint in train_set)))
    
    # We take a very tiny subset for fast demonstration
...
def train():
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # Resample is skipped here for simplicity, assuming 16000 Hz
            optimizer.zero_grad()
            output = model(data)
